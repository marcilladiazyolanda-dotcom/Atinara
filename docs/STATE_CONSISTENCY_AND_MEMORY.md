...
| Corrector Experto | `market-draft-fixer.js` | Edge `market-draft-fixer` → `apply_market_draft_expert_repair` → Edge `validate-market-draft` | `expected_version`; el resultado antiguo no puede guardar | Contenido+binding atómicos; guardado interno con UUID; revisión es un intento separado | UI bloqueada; como máximo una versión; retry viejo falla cerrado | Solo errores de contenido; propuesta, campos y binding auditados; nunca confirma/publica |
...
| Programar / publicar | `admin-markets.js` | `publish_market_draft` / `materialize_market_draft` | Lock, versión, aprobación y confirmación actuales | La materialización e inserción son atómicas; el identificador publicado es único | Botón bloqueado; un estado ya materializado no duplica mercado | Programación/publicación auditada; nunca elude puertas |
