...
- El árbol final de limpieza elimina `data.js`, oculta a invitadas el saldo y progreso de cuenta provisionales y coordina los recursos en `v=20260801-market2`. La escritura automática en GitHub devolvió `403 Resource not accessible by integration`; el commit remoto queda pendiente de publicación manual y no debe inventarse su hash.
...
- Antes de retomar la dirección artística debe terminar la comprobación pública posterior a la limpieza, incluida la aceptación móvil real. Después se vuelve a logo, paleta, tipografías, iconografía, retícula, componentes, avatares, emblemas, movimiento y responsive; no se programa otro bloque funcional.
